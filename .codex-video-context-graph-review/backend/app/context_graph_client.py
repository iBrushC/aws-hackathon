...
from app.constants import SEGMENT_VECTOR_PROPERTY
...
def _serialize(value):
    ...
    if isinstance(value, Node):
        # Drop the raw embedding. Nothing downstream reads it from here — the
        # frontend filters it out of the inspector, and the agent's tools feed
        # their results straight to the LLM, where a 512-float array per Segment
        # burns thousands of tokens and blows the rate limit on real graphs.
        props = {k: v for k, v in value.items() if k != SEGMENT_VECTOR_PROPERTY}
